...
from stages.layout import load_model, detect_layout
...
def process_pdf(
    ...
    Args:
        ...
        odd_only: When True, only OCR odd-numbered pages.
    ...
    if odd_only:
        print("  ↳ Only processing odd-numbered pages (English)")
    print(f"{'=' * 60}")
    ...
    for page_num, img_path in enumerate(page_paths, start=1):
        if not should_process_page(page_num, total_pages, odd_only, skip_covers):
            print(f"Page {page_num}/{total_pages} — skipped")
            continue
        try:
            image = Image.open(img_path).convert("RGB")
            bboxes = detect_layout(image, model)
            raw_questions = extract_text_from_page(image, bboxes, page_num)
            all_raw_questions.extend(raw_questions)
            print(f"Page {page_num}/{total_pages} — {len(raw_questions)} questions found")
        except Exception:
            print(f"ERROR on page {page_num}/{total_pages} ({img_path.name}):")
            traceback.print_exc()
...
def main(argv: list[str]) -> int:
    ...
    for pdf_arg in args.pdfs:
        ...
        if pdf_path.suffix.lower() != ".pdf":
            ...
            continue
        try:
            process_pdf(pdf_path, odd_only=args.odd_only, skip_covers=args.skip_covers)
        except Exception:
