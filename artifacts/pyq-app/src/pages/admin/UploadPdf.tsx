...
export default function UploadPdf() {
  ...
  const [pageFilter, setPageFilter] = useState<"all" | "odd">("all");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      form.append("oddOnly", String(pageFilter === "odd"));
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Ignore the first and last page (title/instruction pages)</p>
              </div>
            </label>

            {/* Page language filter */}
            <div>
              <p className="text-sm text-gray-400 mb-2">For bilingual UPSC papers — which pages to extract?</p>
              <div className="space-y-2">
                {[
                  { value: "all",  label: "All pages",       desc: "Extract every page" },
                  { value: "odd",  label: "Odd pages only",  desc: "Pages 1, 3, 5… — English in UPSC bilingual papers" },
                  ...
                  <label key={value} className="flex items-start gap-3 cursor-pointer group">
                    <input
                      ...
                      name="pageFilter"
                      value={value}
                      checked={pageFilter === value}
                      onChange={() => setPageFilter(value as "all" | "odd")}
                      className="mt-0.5 accent-violet-500"
                    />
                    <div>
                      <p className="text-sm text-white group-hover:text-violet-300 transition">{label}</p>
