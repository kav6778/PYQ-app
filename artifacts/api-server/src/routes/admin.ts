...
function getPagesForBank(pdfStem: string): { pageNumber: number; filename: string }[] {
  ...
  // The pipeline already deleted skipped pages (covers, Hindi pages, etc.)
  // so we simply return all remaining files in sorted order.
  return files.map((filename, idx) => ({ pageNumber: idx + 1, filename }));
}
...
adminRouter.get("/banks/:bankId", async (req: Request, res: Response) => {
  ...
  const pages = pdfStem
    ? getPagesForBank(pdfStem).map(({ pageNumber, filename }) => ({
        pageNumber,
...
adminRouter.post("/upload", upload.single("file"), async (req: Request, res: Response) => {
  ...
  const evenOnly = req.body.evenOnly === "true";
  ...
  if (evenOnly) pythonArgs.push("--even-only");
